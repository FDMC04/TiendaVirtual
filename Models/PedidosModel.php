<?php  
	class PedidosModel extends Mysql
	{
		...
		public function selectPedidos($idpersona = null){
            ...
            $sql = "SELECT p.idpedido,p.referenciacobro,p.idtransaccionpaypal,DATE_FORMAT(p.fecha, '%d/%m/%Y') as fecha, p.monto,tp.tipopago,tp.idtipopago,p.status FROM pedido p INNER JOIN tipopago tp ON p.tipopagoid = tp.idtipopago $where ";
		...
		public function selectPedido(int $idpedido, $idpersona = null){
			...
			if(!empty($requestPedido)){
				...
				$sql_cliente = "SELECT idpersona, nombres, apellidos, telefono, email_user, nit, nombrefiscal, direccionfiscal FROM persona WHERE idpersona = $idpersona ";
		...
		public function selectTransPaypal(string $idtransaccion, $idpersona = null){
			...
			if(!empty($requestData)){
				// Se decodifica el formato json de la respuesta para convertitrlo en un objeto
				$objData = json_decode($requestData['datospaypal']);
				// dep($objData);exit;
				// Se accede al objData hasta llegar al elemento href
				// $urlTransaccion = URLPAYPAL."/v2/payments/captures/".$this->idTransaccion;
		...
		public function reembolsoPaypal(string $idtransaccion, string $observacion){
			...
			$requestData = $this->select($sql);
			if(!empty($requestData)){
				// Se desconvierte el json
				$objData = json_decode($requestData['datospaypal']);
				// Se busca el link para consultar los datos de la transferencia
				$urlOrden = $objData->links[0]->href;
				// Se buscan los datos utilizando la api de paypal
				$objTrans = CurlConnectionGet($urlOrden, "application/json",getTokenPaypal());
				// Una vez encontrados los datos de la transferencia se busca el link que funciona para realizar el reembolso a traves de la api de paypal
				$urlReembolso = $objTrans->purchase_units[0]->payments->captures[0]->links[1]->href;
		...
		public function updatePedido(int $idpedido, $transaccion = null, $idtipopago = null, string $estado){
			if($transaccion == null){
				// Esto es cuando se actualiza un pedido pagado con paypal
				$query_insert = "UPDATE pedido SET status = ? WHERE idpedido = $idpedido";
				$arrData = array($estado);
			}else{
				// Esto es cuando se actualiza un pedido a contra entrega
