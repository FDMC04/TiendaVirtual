<?php  
	...
	class ClientesModel extends Mysql
	{
		...
		{
			$sql = "SELECT idpersona,identificacion,nombres,apellidos,telefono,email_user,password,status
				FROM persona  
		...
		public function selectCliente(int $idpersona){
			...
			$sql = "SELECT idpersona,identificacion,nombres,apellidos,telefono,email_user,nit,nombrefiscal,direccionfiscal,status,DATE_FORMAT(datecreated, '%d-%m-%Y') as fechaRegistro FROM persona WHERE idpersona = $this->intIdUsuario AND rolid = 7";
			// echo $sql; exit();
		...
		public function updateCliente(int $idUsuario, string $identificacion, string $nombre, string $apellido, int $telefono, string $email, string $password, string $nit, string $nomfiscal, string $dirfiscal)
		{
			...
			if(empty($request))
			{
				if($this->strPassword != "")
				{
					...
					$sql = "UPDATE persona SET identificacion=?,nombres=?,apellido=?,telefono=?,email_user=?,nit=?,nombrefiscal=?,direccionfiscal=? WHERE idpersona = $this->intIdUsuario ";
					$arrData = array(
						$this->strIdentificacion,
						$this->strNombre,
						$this->strApellido,
