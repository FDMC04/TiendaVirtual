<?php
	...
	class Login extends Controllers{
				...
				if(empty($_POST['txtEmail']) || empty($_POST['txtPassword'])){
					...
					if(empty($requestUser)){
						...
						if($arrData['status'] == 1){
							...
							$_SESSION['login'] = true;
							// Esta variable es cerrar sesion
							$_SESSION['timeout'] = true;
							// Esta variable guarda el tiempo en el que se inicio sesion
							$_SESSION['inicio'] = time();

							// Esto envia los datos al dashboard
		...
		public function resetPass(){
			if($_POST)
			{
				// Esto hace que no se muestre el error
				error_reporting(0);
					...
					if(empty($arrData))
					{
						...
						if($requestUpdate){
							...
							if($sendEmail){
								...
								$arrReponse = array('status'=>false,'msg'=>'No es posible realizar el proceso, intenta más tarde.');
		...
		public function confirmUser(string $params)
		{
			if(empty($params)){
				...
				if(empty($arrResponse)){
					...
					$data['page_functions_js'] = "functions_login.js";
					$this->views->getView($this,"cambiar_contrasena",$data);
		...
		public function setPassword(){
			if(empty($_POST['idUsuario']) || empty($_POST['txtEmail']) || empty($_POST['txtToken']) || empty($_POST['txtPassword']) || empty($_POST['txtPasswordConfirm'])){
				...
				if($strPassword != $strPasswordConfirm){
					$arrResponse = array('status'=>false,'msg'=>'Las contraseñas no son igual.');
				}else{
