...
window.addEventListener('load', function(){
	...
	if(document.querySelector("#formProductos"))
	{
		...
		formProductos.onsubmit = function(e){
			...
			{
				swal("Atención", "Todos los campos son obligatoris.", "error");
				return false;
			}

			if(intCodigo.length < 5)
			{
				swal("Atención", "El código debe ser mayor que 5 digitos.", "error");
				return false;
			}
...
tinymce.init({
  ...
  plugins: [
    ...
    'anchor', 'pagebreak', 'searchreplace', 'visualblocks', 'visualchars', 'code', 'fullscreen',
    'save', 'insertdatetime', 'media', 'contextmenu', 'directionality', 'nonbreaking', 'emoticons', 'template', 'paste', 'table', 'help', 'wordcount', 'textcolor'
  ],
  toolbar: 'language insertfile undo redo  | styleselect | blocks | ' +
  'bold italic backcolor | alignleft aligncenter ' +
  'alignright alignjustify | bullist numlist outdent indent | link image | print preview media fullpage | forecolor backcolor emoticons ' +
  'removeformat | help',
  content_langs:[
...
function fntEditInfo(element, idproducto){
	...
	document.querySelector("#titleModal").innerHTML = "Actualizar el Producto";
		...
		if(request.readyState == 4 && request.status == 200)
		{
			...
			if(objData.status)
			{
				...
				if(objProducto.images.length > 0){
					...
					for(let p = 0; p < objProductos.length; p++){
						let key = Date.now()+p;
						htmlImage += `<div id="div${key}"><div class="prevImage"><img src="${objProductos[p].url_image}"></img></div><button type="button" class="btnDeleteImage" onclick="fntDelItem('#div${key}')" imgname="${objProductos[p].img}"><i class="fas fa-trash-alt"></i></button></div>`;
					}
				}
...
function openModal(){
	rowTable = "";
	// document.querySelector('#prevPhotoBack').classList.add("prevPhotoBack");
